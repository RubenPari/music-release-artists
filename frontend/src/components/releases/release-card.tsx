    ...
    <div className="group relative overflow-hidden rounded-xl bg-white shadow-sm transition-all hover:shadow-md">
      ...
      <div className="p-4">
        <div className="mb-2 flex items-start justify-between gap-2">
          <h3 className="font-semibold text-foreground line-clamp-1" title={release.title}>
            {release.title}
          </h3>
          <Badge variant={release.type}>{typeLabels[release.type]}</Badge>
        </div>

        <p className="mb-1 text-sm text-muted line-clamp-1">{release.artist.name}</p>
        <p className="mb-3 text-xs text-muted/70">{formatDate(release.releaseDate)}</p>
